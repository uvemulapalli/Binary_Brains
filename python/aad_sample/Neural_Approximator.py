...
def vanilla_net(
    ...
    ws.append(
        tf.get_variable("w" + str(hidden_layers + 1), [hidden_units, 1], initializer=tf.variance_scaling_initializer(),
                        dtype=real_type))
...
def diff_training_graph(
    ...
    loss = alpha * tf.losses.mean_squared_error(labels, predictions) + beta * tf.losses.mean_squared_error(
        derivs_labels * lambda_j, derivs_predictions * lambda_j)
...
class Neural_Approximator():
    ...
    def build_graph(self,
        ...
        with self.graph.as_default():
            ...
            if not differential:
                ...
                self.inputs, self.labels, self.predictions, self.derivs_predictions, self.learning_rate, self.loss, self.minimizer = vanilla_training_graph(
                    self.n, hidden_units, hidden_layers, weight_seed)
                ...
                self.inputs, self.labels, self.derivs_labels, self.predictions, self.derivs_predictions, self.learning_rate, self.loss, self.minimizer = diff_training_graph(
                    self.n, hidden_units, hidden_layers, weight_seed, self.alpha, self.beta, self.lambda_j)
    ...
    def prepare(self,
        ...
        # prepare dataset
        self.x_mean, self.x_std, self.x, self.y_mean, self.y_std, self.y, self.dy_dx, self.lambda_j = normalize_data(
